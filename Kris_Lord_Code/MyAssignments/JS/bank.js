...
function login() {
    ...
    */
    ...
    if (name == null || name.trim() == '' || pass == null || pass.trim() == '') {
        ...
        document.getElementById('messageLogin').innerHTML = "You must fill out form. If you do not have a username and password, please <a href=bank.html>register</a>.";
    }
    else {
        //send request 
        var xhr = new XMLHttpRequest();
        xhr.onreadystatechange = function () {
            ...
            if (xhr.readyState == 4) {
                ...
                if (userArr.length == 0) {
                    //no user w username exists. tell them to try again or register.
                    document.getElementById('messageLogin').innerHTML = "No user with that username exists. Please try again or <a href=bank.html>register.</a>";
                } else {
                    if (userArr[0].password == pass) {
                        //login success, and log their first and last name
                        console.log("Login successful");
                        console.log(`${userArr[0].firstName} ${userArr[0].lastName} is logged in.`);
                        document.getElementById('loginForm').remove();
                        document.getElementById('homePage').removeAttribute('hidden');
                    } else {
                        console.log("Login unsuccessful");
                        document.getElementById('messageLogin').innerHTML = "Incorrect password. Please try again.";
                    }
