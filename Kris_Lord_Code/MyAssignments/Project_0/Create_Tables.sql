...
CREATE OR REPLACE PROCEDURE DEPOSIT (BAL IN NUMBER, UID IN NUMBER, ACCTYPE IN VARCHAR2)
IS
BEGIN
UPDATE BANK_ACCOUNT
SET BALANCE=(BALANCE+BAL)
WHERE USERID=UID AND ACCOUNTTYPE=ACCTYPE;
COMMIT;
END;
/
CREATE OR REPLACE PROCEDURE WITHDRAW (BAL IN NUMBER, UID IN NUMBER, ACCTYPE IN VARCHAR2)
IS
BEGIN
UPDATE BANK_ACCOUNT
SET BALANCE=(BALANCE-BAL)
