...
function login(){
   ...
   for(let prop in user){
       ...
       if(user == null || user.trim() == '' || pass == null || pass.trim() == ''){
        ...
        document.getElementById('messages').innerHTML='Sorry, please fill out form completely';
           
           // send request to get user by user  name validate user
        }
        else{
            //send request. and clear message field
        ...
        if(xhr.readyState == 4){
            ...
            if(userArr.length == 1){
                ...
                if(pass==userArr[0].password){
                    console.log("WHooo");
                    document.getElementById('messages').innerHTML = 'loggedinwhooo';
                
                    newPage(userArr[0].firstName,userArr[0].lastName);
                }
            }
            else{
                
                document.getElementById('messages').innerHTML = 'Sorry, that username is does not exsist try again or create account.';
