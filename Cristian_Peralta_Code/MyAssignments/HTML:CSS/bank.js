...
function login() {
    ...
    if no user with username exists simply let user know their credentials are invisible 
    if user exists but pw is wrong, do the same thing
    
    if user exists and password does match, consol.log success, we will
    later make a homepage for the user. */
    var logUserName = document.getElementById("logUser").value;
    var logPassWord = document.getElementById("logPass").value;

    if (logUserName == null && logPassWord == null || logUserName.trim() == '' && logPassWord.trim() == '') {
        empty = true;
        document.getElementById('message').innerHTML =
            'Sorry, please fill out form completely';
    }
        ...
        xhr.onreadystatechange = function () {
            if (xhr.State == 4) {
                console.log(xhr.status);
                var userArray = JSON.parse(xhttp.responseText);
                if (userArray.length == 0){
                    window.alert("Sorry Wrong User Name or Password");
                    console.log("No user recognized");
                }else{
    ...
    xhr.open("GET", `http://localhost:3000/users?username${user}`);
