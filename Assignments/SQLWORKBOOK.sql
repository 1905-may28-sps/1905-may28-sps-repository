...
CREATE OR REPLACE PROCEDURE GETEMPNAME(EMP_CURS OUT SYS_REFCURSOR)
IS
BEGIN
OPEN EMP_CURS FOR SELECT FIRSTNAME, LASTNAME FROM EMPLOYEE;
END;
/
VAR REFCUS REFCURSOR
EXECUTE GET_FNLNEMP(:REFCUS)
PRINT REFCUS;

--4.2 Stored Procedure Input Parameters
--Task – Create a stored procedure that updates the personal information of an employee.
CREATE OR  REPLACE PROCEDURE UPDATEEMP(EMP_ID IN NUMBER, FNAME IN VARCHAR2)
AS 
BEGIN
UPDATE EMPLOYEE SET FIRSTNAME = FNAME WHERE employeeid = EMP_ID;
END;
/

EXEC UPDATEEMP(6, 'LANCE');
...
CREATE OR REPLACE PROCEDURE Emp_MGR(MG_CURS OUT SYS_REFCURSOR)
IS
BEGIN
OPEN MG_CURS FOR SELECT FIRSTNAME, LASTNAME FROM EMPLOYEE WHERE TITLE LIKE '%Manager';
END;
/
VAR REFCUR REFCURSOR
EXECUTE Emp_MGR(:REFCUR)
PRINT REFCUR;
...
CREATE OR REPLACE PROCEDURE GETCUSTOMERCOMPANY(CUSTCURS OUT SYS_REFCURSOR, CUSTOMERINFO IN NUMBER)
IS
BEGIN
OPEN CUSTCURS FOR SELECT FIRSTNAME, LASTNAME, COMPANY FROM CUSTOMER WHERE CUSTOMERID = CUSTOMERINFO;
END;
/
VAR RCURS REFCURSOR
EXECUTE GETCUSTOMERCOMPANY(:RCURS, 5)
PRINT RCURS;
...
EXECUTE DELETE_INV(23);

SELECT * FROM INVOICELINE WHERE INVOICEID = 23;
SELECT * FROM INVOICE WHERE INVOICEID = 23;


--Task – Create a transaction nested within a stored procedure that inserts a new record in the Customer table
CREATE OR REPLACE PROCEDURE INSERT_INTO_CUSTOMER (
CUSTOMER_ID IN NUMBER, FIRS_TNAME IN VARCHAR2, LAST_NAME IN VARCHAR2, COMPANY_NAME IN VARCHAR2, ADDR IN VARCHAR2, 
CITY_NAME IN VARCHAR2, STATE_NAME IN VARCHAR2, COUNTRY_NAME IN VARCHAR2, POSTAL_CODE IN VARCHAR2,
PHONE_NUMBER IN VARCHAR2, FAX_NUMBER IN VARCHAR2, EMAIL_ADDR IN VARCHAR2, SUPPORTREP_ID IN NUMBER)
AS 
BEGIN 
insert into customer (CUSTOMERID, FIRSTNAME, LASTNAME, COMPANY, ADDRESS, CITY, STATE, COUNTRY, POSTALCODE,
PHONE, FAX, EMAIL, SUPPORTREPID) 
VALUES (CUSTOMER_ID , FIRS_TNAME, LAST_NAME, COMPANY_NAME, ADDR, 
CITY_NAME, STATE_NAME, COUNTRY_NAME, POSTAL_CODE,
PHONE_NUMBER, FAX_NUMBER, EMAIL_ADDR, SUPPORTREP_ID);
END;
/

EXEC insert_into_customer(63, 'MIKE', 'JORDAN', 'Apple', '344 Apple way', 'San Francisco', 'CA', 'United States', 
'34554','897-765-3423', '897-324-6578', 'gfhgjd@gmail.com', 5);
